...
func main() {
	...
	// Initialize DbService for each type
	allergyDbService := db_service.NewMongoService[medcare.AllergyRecord](config, "allergy_records")
	defer allergyDbService.Disconnect(context.Background())

	labResultDbService := db_service.NewMongoService[medcare.LabResult](config, "lab_results")
	defer labResultDbService.Disconnect(context.Background())

	medicalRecordDbService := db_service.NewMongoService[medcare.MedicalRecord](config, "medical_records")
	defer medicalRecordDbService.Disconnect(context.Background())

	vaccinationRecordDbService := db_service.NewMongoService[medcare.VaccinationRecord](config, "vaccination_records")
	defer vaccinationRecordDbService.Disconnect(context.Background())
